...
resource "aws_route_table" "az_A__Private_SubNet_RouteTable" {
  depends_on = [aws_nat_gateway.spicenet[0]]
  vpc_id     = aws_vpc.main.id
  tags = {
  ...
  route {
    cidr_block = "0.0.0.0/0"
    gateway_id = aws_nat_gateway.spicenet[0].id
  }
}

resource "aws_route_table" "az_B__Private_SubNet_RouteTable" {
  depends_on = [aws_nat_gateway.spicenet[1]]
  ...
  route {
    cidr_block = "0.0.0.0/0"
    gateway_id = aws_nat_gateway.spicenet[1].id
  }
}
